...
class SyncJobReportD2Repository(SyncJobReportRepository):
    def __init__(self, instance: Instance, logs_folder: str):
    ...
    def get(self, since: Optional[datetime] = None) -> SyncJobReport:
        ...
        if is_docker_path:
            ...
            with DockerSyncTemporalFolder(container_name, container_path) as docker_logs_folder:
                return D2LogsParser(self.instance, docker_logs_folder).get(since=since)
        else:
            return D2LogsParser(self.instance, self.logs_folder).get(since=since)
